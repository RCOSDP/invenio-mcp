...
- **受け取ったトークンをキーに使うのをやめ、期限切れを捨てるようになった。** 検証結果
  ...
  要約に変え、引くたびに期限切れを捨てるようにした。TTL は単調増加の時計で測るので、
  壁時計が動いても期限切れが使えたり生きている項目が早く消えたりしない。掃除は実際に
  期限が切れたものだけを取り出すので、キャッシュが埋まっても引く操作は重くならない。
  挙動は変わらない。
